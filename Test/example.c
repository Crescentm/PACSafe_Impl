 ...
 *    BadSink : Use a block of memory returned from a function after it has been
 * freed Flow Variant: 01 Baseline
 *
 * */
...
static char *helperBad(char *aString) {
  ...
  if (aString != NULL) {
    ...
    reversedString[i] = '\0';
    /* FLAW: Freeing a memory block and then returning a pointer to the freed
     * memory */
    free(reversedString);
...
void CWE416_Use_After_Free__return_freed_ptr_01_bad() {
  {
    ...
    /* free(reversedString);
     * This call to free() was removed because we want the tool to detect the
     * use after free, but we don't want that function to be free(). Essentially
...
void CWE416_Use_After_Free__return_freed_ptr_01_good() { good1(); }
