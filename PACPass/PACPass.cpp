...
// String references for pac functions.
StringRef INITMEMORY("init_memory");
StringRef ADDPAC("addPAC");
StringRef AUTHPAC("authPAC");
...
// Statistics regarding instructions and to count number of times each function
// is instrumented.
#define DEBUG_TYPE "PACPass"
...
STATISTIC(numPACStores, "Number of memory stores protected with PAC-Pass.");
STATISTIC(numLoads, "Number of memory loads.");
STATISTIC(numPACLoads, "Number of memory loads protected with PAC-Pass.");
STATISTIC(numMemcpy, "Number of memcpy operations.");
STATISTIC(numPACMemcpy, "Number of memcpy operations protected with PAC-Pass.");
STATISTIC(numAllocFree, "Number of malloc and free operations.");
STATISTIC(numPACAllocFree,
          "Number of malloc and free operations protected with PAC-Pass.");
STATISTIC(numInitStores, "Number of initialization memory stores.");
STATISTIC(numPACInitStores,
          "Number of initialization memory stores protected with PAC-Pass.");
...
// A list of all legitimate indirect functions used by the program. These
// require metadata to be stored.
...
// A list of all values that require a PAC to be re-added to the pointer after
// authentication. This is a temporary solution, at least for function pointers
// until we add BLRAA.
SetVector<Value *> AddPACDerefValues;

// A list of all values that need to have removeMetadata instrumented before
// them. These are mainly deallocations (free, munmap) and return addresses.
...
namespace {
class PACPass : public ModulePass {
public:
  static char ID;
  PACPass() : ModulePass(ID) {}
  ~PACPass() {}
  ...
  void checkIndFunction(Module &M) {
    for (auto &F : M) {
      for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
        BasicBlock *BB = &*FIt;
        for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end();
             ++BBIt) {
          Instruction *I = &*BBIt;
          int i = 0;
          if (StoreInst *SI = dyn_cast<StoreInst>(I)) {
            ...
            Type *VTy = V->getType();
            if (VTy->isPointerTy() && cast<PointerType>(VTy)
                                          ->getPointerElementType()
                                          ->isFunctionTy()) {
              SetMetaIndValues[I] = F.getName();
            }

            for (auto SetMIt = SetMetaIndValues.begin();
                 SetMIt != SetMetaIndValues.end(); ++SetMIt) {
        ...
        if (AI) {
          ...
          if (CF && !CI->arg_empty()) {
            ...
            if (shouldProtectType(VArg->getType())) {
              if (GetElementPtrInst *GEPArg =
                      dyn_cast<GetElementPtrInst>(VArg)) {
                AuthPACValues.insert(I);
              }
            }
          }

          // look for the store instruction after the allocation,
          // and check if the pointer is sensitive. If it is, we add it to our
          // setMeta set (setMetadata for the allocated object). We also remove
          // the value that was set on the alloca (stack).
          if (CF && isAllocationFn(CI, TLI)) {
            ++numAllocFree;
            ...
            if (isa<StoreInst>(nextStoreInst)) {
              ...
              if (ConstantExpr *CE =
                      dyn_cast<ConstantExpr>(SI->getValueOperand())) {
              ...
              if (shouldProtectType(VTy)) {
                ++numPACAllocFree;
                for (auto SVIt = SetMetaValues.begin();
                     SVIt != SetMetaValues.end(); ++SVIt) {
                ...
                if (!isCPS && numElements == 1) {
                  ...
                  for (unsigned int i = 0; i < numElements; i++) {
                    Value *VA = insertArrayGEP(
                        nextStoreInst->getNextNode(), i,
                        VSI->getType()->getPointerElementType(), VL);
          ...
          // Here we are adding the PAC again after the sensitive function call.
          // Before the function is called it has to be loaded, and so we check
          // if the previous instruction is a load and if it's PointerOperand is
          // sensitive. We know that the PAC has already been authenticated.
          ...
          // Handling memcpy
              ...
              if (LoadInst *LI = dyn_cast<LoadInst>(BI->getOperand(0))) {
                if (shouldProtectType(LI->getPointerOperand()->getType())) {
                  removeMetaValues.insert(I);
                           ...
                           dyn_cast<LoadInst>(CI->getArgOperand(0))) {
              if (shouldProtectType(LI->getPointerOperand()->getType())) {
                removeMetaValues.insert(I);
              }
            }
            ...
            if (Function *CF = dyn_cast<Function>(CE->getOperand(0))) {
              if (isDeallocLibFunction(CF->getName())) {
                if (BitCastInst *BI =
                        dyn_cast<BitCastInst>(CI->getArgOperand(0))) {
                  if (LoadInst *LI = dyn_cast<LoadInst>(BI->getOperand(0))) {
                    if (shouldProtectType(LI->getPointerOperand()->getType())) {
                      removeMetaValues.insert(I);
                    }
          ...
          ++numStores;
          if (GlobalValue *GV =
                  dyn_cast<GlobalValue>(SI->getPointerOperand())) {
            if (GV->getName().startswith("__") ||
          ...
          if (GlobalValue *GVV = dyn_cast<GlobalValue>(SI->getValueOperand())) {
            if (GVV->getName().startswith("__") ||
                GVV->getSection().startswith("llvm")) {
              continue;
            }
          }
          ...
          if (isCPS) {
            ...
            if (BitCastInst *BSI = dyn_cast<BitCastInst>(VSI)) {
              ...
              if (CallInst *CSI = dyn_cast<CallInst>(BO)) {
                if (isMallocOrCallocLikeFn(CSI, TLI)) {
                  ...
                  unsigned int numElements = allocSize / typeSize;
                  if (numElements == 1 &&
                      shouldProtectTypeFP(BSI->getDestTy())) {
                    CPSValues.insert(I);
                  }
                }
              }
          ...
          if (VRHS->getName() == "incdec.ptr" ||
              VRHS->getName().startswith("__")) {
            continue;
          }
          ...
          if (isCPS && !noInstrument) {
            if (AllocaInst *AI = dyn_cast<AllocaInst>(V)) {
              for (auto SetMIt = SetMetaValues.begin();
                   SetMIt != SetMetaValues.end(); ++SetMIt) {
                  ...
                  if (AllocaInst *ASI =
                    ...
                    if (ASI == AI) {
                      ...
                      if (shouldProtectTypeFP(ATy)) {
                        if (StoreInst *SI =
                                dyn_cast<StoreInst>(I->getNextNode())) {
                ...
                } else if (AllocaInst *ASI =
                               dyn_cast<AllocaInst>(SetMIt->first)) {
                  if (ASI == AI) {
                    Type *ATy = AI->getAllocatedType();
                    if (shouldProtectTypeFP(ATy)) {
                      if (StoreInst *SI =
              ...
              if (GTy->isPointerTy() && cast<PointerType>(GTy)
                                            ->getPointerElementType()
                                            ->isFunctionTy()) {
                CPSValues.insert(I);
              }
          ...
          else if ((shouldProtectType(Ty) && !noInstrument &&
                    LI->getName() != "gep_load") ||
                   (isBitcastIntoSensitive(LI) && !noInstrument &&
                    LI->getName() != "gep_load")) {
            ++numPACLoads;
            if (V == LI->getPrevNonDebugInstruction()) {
              if (LoadInst *LII =
                      dyn_cast<LoadInst>(LI->getPrevNonDebugInstruction())) {
          ...
          if (shouldProtectType(SrcTy) && !shouldProtectType(DestTy)) {
            if (LoadInst *LBI = dyn_cast<LoadInst>(VBI)) {
              if (isa<AllocaInst>(LBI->getPointerOperand()) ||
                  isa<GlobalValue>(LBI->getPointerOperand())) {
                continue;
              }
            }
            if (isa<CallInst>(VBI)) {
  ...
  // Finds if there are any stores for the operand of the instruction. This is
  // necessary to allow addPAC to be instrumented after the load if there are no
  // stores.
  ...
  Value *insertArrayGEP(Instruction *insertPoint, unsigned int index, Type *Ty,
                        Value *V) {
    LLVMContext &C = insertPoint->getFunction()->getContext();
    IRBuilder<> IRB(C);
  ...
  void insertGlobalStructMemberGEP(Instruction *insertPoint, StructType *STy,
                                   Value *V, int typeSize) {
    unsigned numElements = STy->getNumElements();
    for (unsigned i = 0; i < numElements; i++) {
      if (StructType *ASTy = dyn_cast<StructType>(STy->getElementType(i))) {
        ...
        insertGlobalStructMemberGEP(insertPoint->getNextNode(), ASTy, VS,
                                    typeSize);
      }
    }
  }

  void recursiveStructInsertions(Value *VS, Value *VGEP, Instruction *I,
                                 StringRef StLibCall) {
    if (LoadInst *GLI = dyn_cast<LoadInst>(VGEP)) {
      ...
      if (GetElementPtrInst *VGI = dyn_cast<GetElementPtrInst>(VGLI)) {
        recursiveStructInsertions(VGI, VGI->getPointerOperand(),
                                  cast<Instruction>(CL), StLibCall);
      }
    }
      ...
      if (shouldProtectType(BI->getSrcTy())) {
        if (LoadInst *GLI = dyn_cast<LoadInst>(BI->getOperand(0))) {
          ...
          recursiveStructInsertions(GGI, GGI->getPointerOperand(), I,
                                    StLibCall);
        }
      }
    }
  ...
  // Check to find if struct is sensitive, in case a non-sensitive store is
  // used. This is necessary to addPAC to sensitive structs when they use
  // non-sensitive members.
  ...
  Value *authenticateStructMembers(Instruction *insertPoint, StructType *STy,
                                   Value *V) {
    unsigned numElements = STy->getNumElements();
    LLVMContext &C = insertPoint->getFunction()->getContext();
    IRBuilder<> IRB(C);
    ...
    LoadInst *load = IRB.CreateLoad(V->getType(), V, "load_tmp");
  ...
  bool isI8Pointer(Value *V) {
    if (PointerType *PTy = dyn_cast<PointerType>(V->getType())) {
      if (PointerType *PTy1 =
              dyn_cast<PointerType>(PTy->getPointerElementType())) {
        if (IntegerType *ITy =
                dyn_cast<IntegerType>(PTy1->getPointerElementType())) {
  ...
  bool shouldProtectType(Type *Ty) {
    // Function pointer
    if (Ty->isPointerTy() &&
        cast<PointerType>(Ty)->getPointerElementType()->isFunctionTy()) {
      return true;
    }
  ...
  // Insert Set Meta Library call. This is because setMetadata takes three
  // arguements.
  void insertSetMetaLibCall(Value *ArgVal, Instruction *InsertPointInst,
                            StringRef FunctionName, unsigned int Elements,
                            int typeSize) {
    ...
    ArrayRef<Type *> FuncParams = {Ty->getPointerTo(), NumElements->getType(),
                                   TypeSize->getType()};
    ...
    FunctionCallee Func =
        InsertPointInst->getModule()->getOrInsertFunction(FunctionName, FTy);
    Value *Addr = builder.Insert(
        CastInst::CreatePointerCast(ArgVal, Ty->getPointerTo(), ""));
    ArrayRef<Value *> FuncArgs = {Addr, NumElements, TypeSize};

    builder.CreateCall(Func, FuncArgs);
  }

  // Insert Library call.
  CallInst *insertLibCallAfter(Value *ArgVal, Instruction *InsertPointInst,
    ...
    ArrayRef<Type *> FuncParams = {Ty->getPointerTo()};
  ...
  // Insert Library call.
  CallInst *insertLibCallBefore(Value *ArgVal, Instruction *InsertPointInst,
                                StringRef FunctionName) {
    LLVMContext &C = InsertPointInst->getFunction()->getContext();
    IRBuilder<> builder(C);
  ...
  CallInst *insertLibCallAuth(Value *ArgVal, Instruction *InsertPointInst,
                              StringRef FunctionName, unsigned int arrayIndex) {
    LLVMContext &C = InsertPointInst->getFunction()->getContext();
    IRBuilder<> builder(C);
    ...
    ArrayRef<Type *> FuncParams = {Ty->getPointerTo(), ArrayIndex->getType()};
    ...
    FunctionCallee Func =
        InsertPointInst->getModule()->getOrInsertFunction(FunctionName, FTy);
    ArrayRef<Value *> FuncArgs = {
        builder.CreatePointerCast(ArgVal, Ty->getPointerTo()), ArrayIndex};
  ...
  void instrumentSetMeta(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        Instruction *I = &*BBIt;
        for (auto SetMIt = SetMetaValues.begin(); SetMIt != SetMetaValues.end();
             ++SetMIt) {
          if (I == SetMIt->first) {
            if (AllocaInst *AI = dyn_cast<AllocaInst>(SetMIt->first)) {
              Value *V = &*AI;
              if (V == I) {
                insertSetMetaLibCall(V, I, SETMETA, SetMIt->second.numElements,
                                     SetMIt->second.typeSize);
                ++setMeta_count;
              }
              ...
              if (StoreInst *SII = dyn_cast<StoreInst>(I)) {
                ...
                if (V == VS) {
                  insertSetMetaLibCall(V, I, SETMETAOBJ,
                                       SetMIt->second.numElements,
                                       SetMIt->second.typeSize);
                  ++setMeta_count;
                }
              }
                         ...
                         dyn_cast<GetElementPtrInst>(SetMIt->first)) {
  ...
  void instrumentSetMetaNull(Function &F) {
    if (F.getName() == "main") {
      ...
      Value *V = llvm::Constant::getNullValue(
          llvm::Type::getInt8Ty(C)->getPointerTo());
  ...
  void instrumentAddPAC(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        Instruction *I = &*BBIt;
        for (auto AddPIt = AddPACValues.begin(); AddPIt != AddPACValues.end();
             ++AddPIt) {
          if (I == *AddPIt) {
            if (LoadInst *LI = dyn_cast<LoadInst>(*AddPIt)) {
              ...
              if (LoadInst *LII = dyn_cast<LoadInst>(I)) {
                ...
                if (V == VI && !isa<GetElementPtrInst>(I->getNextNode())) {
                  ...
                  if (GetElementPtrInst *GEPI =
                          dyn_cast<GetElementPtrInst>(checkLastGEP(I))) {
                    LoadInst *GEPLI =
                        dyn_cast<LoadInst>(GEPI->getPointerOperand());
                    if (GEPLI) {
                      AllocaInst *GEPAI =
                          dyn_cast<AllocaInst>(GEPLI->getPointerOperand());
                      if (GEPAI && VI == GEPAI) {
                        isGEP = true;
                      }
                    }
                ...
                } else if (V == VI && isCheckIfStore == 1 &&
                           isa<AllocaInst>(LII->getPointerOperand())) {
                  if (isReplace == false) {
              ...
              if (GetElementPtrInst *GRHS = dyn_cast<GetElementPtrInst>(VRHS)) {
                ...
                if (shouldProtectType(TyGRHS)) {
                  if (LoadInst *LIGRHS =
                          dyn_cast<LoadInst>(GRHS->getPointerOperand())) {
                    ...
                    continue;
                  } else if (BitCastInst *BRHS = dyn_cast<BitCastInst>(
                                 GRHS->getPointerOperand())) {
                    if (LoadInst *LIBRHS =
                            dyn_cast<LoadInst>(BRHS->getOperand(0))) {
                ...
                if (GetElementPtrInst *GI = dyn_cast<GetElementPtrInst>(V)) {
                  ...
                  if (isa<StructType>(GI->getSourceElementType())) {
                    if (shouldProtectType(SI->getPointerOperand()->getType())) {
                      ...
                      recursiveStructInsertions(V, VGEP, cast<Instruction>(SI),
                                                ADDPAC);
                    }
                    break;
                  }

                  else if (LoadInst *GLI =
              ...
              if (isMemcpyFunction(CF->getName())) {
                ...
                if (shouldProtectType(realDestType)) {
                  if (BitCastInst *BCIdest = dyn_cast<BitCastInst>(dest)) {
                    if (GetElementPtrInst *GEPdest =
                            dyn_cast<GetElementPtrInst>(
                                BCIdest->getOperand(0))) {
                      if (LoadInst *LIdest = dyn_cast<LoadInst>(
                              GEPdest->getPointerOperand())) {
                        insertLibCallAfter(LIdest->getPointerOperand(), I,
                                           ADDPAC);
                        ++addPAC_count;
                      }
                    } else if (LoadInst *LIdest =
                                   dyn_cast<LoadInst>(BCIdest->getOperand(0))) {
                      insertLibCallAfter(LIdest->getPointerOperand(), I,
                                         ADDPAC);
                      ++addPAC_count;
                    }
                ...
                if (shouldProtectType(realSrcType)) {
                  if (BitCastInst *BCIsrc = dyn_cast<BitCastInst>(src)) {
                    if (GetElementPtrInst *GEPsrc = dyn_cast<GetElementPtrInst>(
                            BCIsrc->getOperand(0))) {
                      if (LoadInst *LIsrc =
                              dyn_cast<LoadInst>(GEPsrc->getPointerOperand())) {
                        insertLibCallAfter(LIsrc->getPointerOperand(), I,
                                           ADDPAC);
                        ++addPAC_count;
                      }
                    } else if (LoadInst *LIsrc =
                                   dyn_cast<LoadInst>(BCIsrc->getOperand(0))) {
  ...
  void instrumentAddPACDeref(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        Instruction *I = &*BBIt;
        for (auto AddPACDIt = AddPACDerefValues.begin();
             AddPACDIt != AddPACDerefValues.end(); ++AddPACDIt) {
          if (I == *AddPACDIt) {
            ...
            if (LoadInst *LI = dyn_cast<LoadInst>(V)) {
              ...
              if (LoadInst *LII = dyn_cast<LoadInst>(VS)) {
                ...
                ++addPAC_count;
                           ...
                           dyn_cast<GetElementPtrInst>(VS)) {
                Value *VGEP = GI->getPointerOperand();
                if (isa<StructType>(GI->getSourceElementType())) {
                  CallInst *CL = insertLibCallAfter(VS, I, ADDPAC);
                  ++addPAC_count;
                  recursiveStructInsertions(VS, VGEP, cast<Instruction>(CL),
                                            ADDPAC);
                }
              ...
              if (LoadInst *BLI = dyn_cast<LoadInst>(VB)) {
                ...
                if (GetElementPtrInst *GI = dyn_cast<GetElementPtrInst>(VBLI)) {
                  ...
                  if (isa<StructType>(GI->getSourceElementType())) {
                    ...
                    ++addPAC_count;
                    recursiveStructInsertions(VBLI, VGEP, cast<Instruction>(CL),
                                              ADDPAC);
                  } else if (LoadInst *GLI = dyn_cast<LoadInst>(VGEP)) {
                    Value *VGLI = GLI->getPointerOperand();
                    insertLibCallAfter(VGLI, I, ADDPAC);
  ...
  void instrumentAuthPAC(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        Instruction *I = &*BBIt;
        for (auto AuthPIt = AuthPACValues.begin();
             AuthPIt != AuthPACValues.end(); ++AuthPIt) {
          if (I == *AuthPIt) {
  ...
  void instrumentCPS(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        Instruction *I = &*BBIt;
        for (auto CPSIt = CPSValues.begin(); CPSIt != CPSValues.end();
             ++CPSIt) {
          if (I == *CPSIt) {
  ...
  void instrumentRemoveMeta(Function &F) {
    for (Function::iterator FIt = F.begin(); FIt != F.end(); ++FIt) {
      ...
      for (BasicBlock::iterator BBIt = BB->begin(); BBIt != BB->end(); ++BBIt) {
        ...
        for (auto removeMetaIt = removeMetaValues.begin();
             removeMetaIt != removeMetaValues.end(); ++removeMetaIt) {
          if (I == *removeMetaIt) {
        ...
        if (isa<ReturnInst>(I)) {
          for (auto SetMt = SetMetaValues.begin(); SetMt != SetMetaValues.end();
               ++SetMt) {
  ...
  void instrumentGlobals(Module &M) {
    ...
    Function *FSetMeta =
        Function::Create(FunctionType::get(Type::getVoidTy(C), false),
                         GlobalValue::InternalLinkage, "globalInstrument", &M);
          ...
          if (!globalsInstrumented) {
            for (auto GIt = M.global_begin(); GIt != M.global_end(); ++GIt) {
              ...
              if (GV && GV->hasInitializer()) {
                ...
                int typeSize = DL.getTypeAllocSize(Ty);
                if (isCPS) {
                  if (shouldProtectTypeFP(Ty) &&
                      !GV->getName().startswith("llvm.") &&
                      !GV->getName().startswith("__")) {
                    ++numPACInitStores;
                  }
                } else {
                  if (shouldProtectType(Ty) &&
                      !GV->getName().startswith("llvm.") &&
                      !GV->getName().startswith("__")) {
                    ...
                    if (ArrayType *ATy = dyn_cast<ArrayType>(Ty)) {
                      ...
                      for (unsigned int i = 0; i < numElements; i++) {
                        ...
                        insertSetMetaLibCall(V, I, SETMETAOBJ, numElements,
                                             typeSize);
                        ++setMeta_count;
                      }
                    }
                      ...
                      Value *TypeSize =
                          ConstantInt::get(Type::getInt64Ty(C), typeSize);
                      ArrayRef<Type *> FuncParams = {Ty->getPointerTo(),
                                                     NumElements->getType(),
                                                     TypeSize->getType()};
                      ...
                      Type *ResultFTy = Type::getVoidTy(C);
                      FunctionType *FTy =
                          FunctionType::get(ResultFTy, FuncParams, false);
                      FunctionCallee Func =
                          M.getOrInsertFunction(SETMETAOBJ, FTy);
                      ArrayRef<Value *> FuncArgs = {
                          builder.CreatePointerCast(GV, Ty->getPointerTo()),
                          NumElements, TypeSize};

                      FunctionType *FTy1 =
                          FunctionType::get(ResultFTy, FuncParams1, false);
                      FunctionCallee Func1 =
                          M.getOrInsertFunction(ADDPAC, FTy1);
  ...
  void recursiveStructReplaceInsertions(Value *VS, Value *VGEP,
                                        Instruction *I) {
    if (LoadInst *GLI = dyn_cast<LoadInst>(VGEP)) {
      Value *VGLI = GLI->getPointerOperand();
      ...
      if (GetElementPtrInst *VGI = dyn_cast<GetElementPtrInst>(VGLI)) {
        recursiveStructReplaceInsertions(VGI, VGI->getPointerOperand(),
                                         cast<Instruction>(CL));
      }
    }
  ...
  CallInst *insertReplaceLibCall(Value *ArgVal, Value *ArgVal_sign,
    ...
    ArrayRef<Type *> FuncParams = {Ty->getPointerTo(), Ty->getPointerTo()};
    ...
    Value *Addr = builder.Insert(
        CastInst::CreatePointerCast(ArgVal, Ty->getPointerTo(), ""));
    Value *Addr_sign = builder.Insert(
        CastInst::CreatePointerCast(ArgVal_sign, Ty->getPointerTo(), ""));
    ArrayRef<Value *> FuncArgs = {Addr, Addr_sign};

    return builder.CreateCall(Func, FuncArgs);
  }

  // Inserts the replaceWithSignedPointer Function. V is the pointer that is
  // replaced with the PACed.
  CallInst *replaceWithSignedPointer(Instruction *insertPoint, LoadInst *LI,
                                     Value *V) {
    if (LI->getPrevNonDebugInstruction() != nullptr) {
      if (CallInst *AuthC =
              dyn_cast<CallInst>(LI->getPrevNonDebugInstruction())) {
        ...
        if (CF) {
          ...
          if (S.equals("authenticatePAC")) {
            ...
            if (Instruction *authVI = dyn_cast<Instruction>(authV)) {
              ...
              CallInst *CLR =
                  insertReplaceLibCall(V, authVL, insertPoint, REPLACESIGN);
              ++numReplace_count;
              return CLR;
            }
          }
            ...
            if (LoadInst *LI = dyn_cast<LoadInst>(V)) {
              ...
              if (LoadInst *LII = dyn_cast<LoadInst>(VS)) {
                ...
                if (isa<StructType>(GI->getSourceElementType())) {
                  ...
                  recursiveStructReplaceInsertions(VS, VGEP,
                                                   cast<Instruction>(CL));
                } else if (LoadInst *GLI = dyn_cast<LoadInst>(VGEP)) {
                  Value *VGLI = GLI->getPointerOperand();
                  replaceWithSignedPointer(I, GLI, VGLI);
              ...
              if (LoadInst *BLI = dyn_cast<LoadInst>(VB)) {
                ...
                if (GetElementPtrInst *GI = dyn_cast<GetElementPtrInst>(VBLI)) {
                  ...
                  if (isa<StructType>(GI->getSourceElementType())) {
                    ...
                    recursiveStructReplaceInsertions(VBLI, VGEP,
                                                     cast<Instruction>(CL));
                  } else if (LoadInst *GLI = dyn_cast<LoadInst>(VGEP)) {
                    Value *VGLI = GLI->getPointerOperand();
                    replaceWithSignedPointer(I, GLI, VGLI);
  ...
  bool isMallocFunction(StringRef s) { return s.equals("malloc"); }

  bool isFreeFunction(StringRef s) { return s.equals("free"); }

  bool isCallocFunction(StringRef s) { return s.equals("calloc"); }

  bool isReallocFunction(StringRef s) { return s.equals("realloc"); }

  bool isMemcpyFunction(StringRef s) { return s.startswith("llvm.memcpy"); }

  bool isMmapFunction(StringRef s) { return s.equals("mmap"); }

  bool isMunmapFunction(StringRef s) { return s.equals("munmap"); }

  bool isAllocLibFunction(StringRef s) {
    return isMallocFunction(s) || isCallocFunction(s) || isReallocFunction(s) ||
           isMmapFunction(s);
  }
